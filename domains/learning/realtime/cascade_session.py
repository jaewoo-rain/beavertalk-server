...
class CascadeSession:
    ...
    async def _run_reply(self, user_text: str, is_greeting: bool = False,
        ...
        try:
            ...
            if self._tts_engine == _GEMINI_BATCH_CHOICE:
                ...
                logger.info(
                    "cascade 대답%s(배치): turn=%s %s %s %s %s %s 글자=%d%s 자막=%d개 "
                    "tts=%s %s %s",
                    "(선톡)" if is_greeting else "", turn_id, timing.summary(),
                    self._llm_tokens_log(chat.usage_metadata),
                    self._emotion_log(),
                    # ⚠ 배치 경로는 문장 수를 따로 안 센다 — 마커 수가 그 자리의 값이다.
                    self._emotion_tag_log(chat.text, self._sentence_markers),
                    self._rate_log(), spoken_chars,
            ...
            logger.info(
                ...
                "cascade 대답%s: turn=%s %s %s %s %s %s 글자=%d%s 자막=%d개 tts=%s "
                "언어분할=%s gemini호출=%d %s %s %s %s %s %s %s %s",
                "(선톡)" if is_greeting else "", turn_id, timing.summary(),
                # ⭐ prefill 이 `LLM첫조각` 의 얼마인지는 이 값 없이는 못 본다.
                self._llm_tokens_log(chat.usage_metadata),
                self._emotion_log(),
                # ⭐⭐ **모델이 태그를 몇 개 내나** — 우리는 첫 태그만 쓴다. 그래서 `감정=` 이
                #   neutral 80% 인 것이 모델 탓인지 **우리가 버려서**인지 이 값 없이는 못 가른다.
                self._emotion_tag_log(chat.text, sentences_done),
                self._rate_log(), spoken_chars,
    ...
    @staticmethod
    def _emotion_tag_log(text: str, sentences: int) -> str:
        """⭐⭐ **모델이 태그를 몇 개 내나** — 문장별 감정을 만들지 말지가 여기서 갈린다.

        실측 `감정=` 분포가 68턴 중 neutral 55(80%)였다. 그런데 우리는 **첫 태그만** 쓴다
        ⇒ 그 80% 가 "모델이 하나만 냈다"인지 "우리가 뒤를 버렸다"인지 **구분이 안 됐다.**
        판정 기준(bt-back):
            태그 1개가 대부분   → 지금이 맞다. 문장별 감정은 값어치 없다
            태그 2개+ 가 상당수 → 우리가 버리고 있었다. 그때 문장별을 만든다
        ⚠ **문장 수를 같이 찍는다** — 태그 3개/문장 3개와 태그 3개/문장 1개는 전혀 다른 사실이다.
        ⚠ 0개는 `-` 다(0 으로 찍지 않는다 — 오늘 이미 정한 규칙).
        ⛔ 대사 원문은 안 남긴다. **태그 이름만.**
        """
        tags = read_emotion_tags(text)
        if not tags:
            return "감정태그=- 문장=%d개" % sentences
        return "감정태그=%d개[%s] 문장=%d개" % (len(tags), ",".join(tags), sentences)
