...
class ClientStart(BaseModel):
    ...
    # ⭐ 개발자도구 «Premium 으로 통화 / Free 로 통화»(2026-09-13 사장님, D1 로 2단화): **관리자(member.role == "admin") 에 한해** 통화 엔진
    #   선택(영상/음성 · 모델 · 백엔드)을 이 플랜 기준으로 정한다. 한도·결제·구독 로직은 무변경(플랜 «흉내» 는 엔진 선택에만). 관리자가
    #   아니면 무시(본인 플랜). 이어하기 조각에도 앱이 같은 값을 다시 보낸다(force_course 와 같은 필드 규율) — 서버는 조각마다 다시 적용한다.
    plan_override: Literal["free", "premium"] | None = None
...
# ⛔⛔ P2-4(2026-09-24, bt-back QA) — 죽은 설명. 아래 `AecHint`/`has_echo_cancel`
#   /`AEC_MODES_WITH_CANCEL` 은 **어디서도 안 읽힌다**(grep 확인 — `has_echo_cancel`
#   소비처 0건). `ClientStart` 에 `aec` 필드 자체가 없다 — 하네스/브라우저 데모는
#   지금도 start 페이로드에 `aec`를 여전히 보내지만(`ctrl.get("aec")`류 옛 경로의
#   흔적), 그 값은 파싱조차 안 되고(`extra="ignore"`) 조용히 버려진다. 예전엔
#   이 값이 실제로 barge-in 확인 방식을 고르거나(느슨한 힌트) 서버 방어 관문을
#   끄는 스위치였을 수 있으나, 지금은 그 관문 자체가 어디에도 없다 — 이 클래스는
#   "쓰이지 않는 계산"이다(도달 불가가 아니라 아무도 안 읽는다는 뜻).
#   지우지 않은 이유: 실제 삭제·재배선은 이 커밋(P2-4, 주석만)의 범위 밖 — 정리는
#   P2-6(고아 코드) 몫으로 미뤘다.
