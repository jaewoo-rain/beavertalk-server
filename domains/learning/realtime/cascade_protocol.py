...
class CascadeSentenceMarker(BaseModel):
    ...
    ⚠ `seq` 는 **마커 순번**이다(그 통화에서 몇 번째 마커인가). 문장 순번도 구간 순번도 아니다:
      · 코드스위칭 문장 하나가 언어 구간 2개면 마커 2개(`text` 는 각각 조각, 프론트가 이어 붙인다)
      · ⭐ 그리고 **한 구간에 문장이 여럿이면 문장 수만큼 마커가 나간다**(2026-08-16) —
        문장별 감정을 실으려면 마커도 문장 단위여야 한다. TTS 요청은 **여전히 구간 단위**다.
      ⇒ 즉 마커는 **늘기만** 했고 순서·의미는 그대로다. 프론트는 도착 순서대로 큐에 꽂고
        연속 마커의 `server_bytes` 차분으로 그 조각의 길이를 잡는데, 조각이 잘게 나뉠 뿐
        계산은 같다 ⇒ **클라 변경 없이 그대로 돈다**(`normalcall_controller._fireDueMarkers`).
    ⚠ 한 구간 안의 두 번째 문장부터는 `server_bytes` 가 **추정 위치**다. TTS 가 구간 하나에
      오디오 하나를 주고 문장 정렬을 안 주기 때문이다(정확히 알려면 문장별로 따로 요청해야
      하는데, 그러면 짧은 문장 뒤에 벤더 왕복이 통째로 공백이 된다). 서버가 그 추정 오차를
      `자막오차=` 로 남긴다.
