...
class TtsSpeechIn(BaseModel):
    ...
    # ⭐ **엔진만은 클라가 고른다.** 목소리·언어와 달리 엔진은 «어느 화면이냐» 의 문제라
    #   서버가 회원 정보로 유도할 수 없다. 안 주면 종전대로 Chirp3-HD 다.
    #   `"gemini-tts"` 는 취약 발음 학습이 쓴다(2026-09-21 사장님 지시).
    #   ⚠ 모르는 값이면 무시하고 기본 엔진으로 간다 — 오타로 소리가 안 나면 안 된다.
