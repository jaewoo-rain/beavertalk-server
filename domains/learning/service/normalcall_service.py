...
def finalize_call(
    ...
    ⭐⭐ C4 재설계(2026-09-23, bt-back A) — `total_time` 은 이제 **기본적으로 건드리지
      ...
      쓴 시간을 확정해 뒀기 때문이다.
      ⛔⛔ P2-4(2026-09-24, bt-back QA) — 이 문단이 한때 "명시로 넘기는 유일한 호출부는
      `cascade_session.py`" 라고 적었는데, 그 파일은 캐스케이드 엔진 삭제로 **이미
      없다**(grep 확인 — `.py` 소스 0건, `.pyc` 캐시만 남음). 지금 이 함수의 실제
      호출부는 `call_session.py`(`_persist_remaining`) 딱 하나이고, 그 자리도
      `total_time` 을 **안 넘긴다**(기본값 None 그대로 씀) — 즉 이 매개변수를 명시로
      채우는 곳은 지금 0건이다(파라미터 자체는 남겨 둔다, 삭제는 별도 판단).
