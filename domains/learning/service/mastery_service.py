...
def _mastered_conditions_met(
    ...
    ⚠ **대가**: **틀린 채로 끝난 통화에서도 마스터가 확정된다.** 마지막 발화가 F 여도
      나머지가 차 있으면 그 통화에서 mastered 로 올라간다.

    ## ⚠ 그래도 F 는 무해해지지 않는다 — **어디서 여전히 무는지** 적어 둔다
    G4(af17e72)에 이어 ④까지 뺐으니 "이제 F 가 아무것도 안 하나?"를 다음 사람이 반드시
    묻는다. 코드로 확인한 답은 **아니오**다:
      · ⭐ **①이 사실상 그 역할을 한다.** F 는 점수 −1.0 이라 score 가 3.0 아래로 떨어지면
        ①에서 막힌다. ⇒ ④ 제거의 실질 효과는 **"score 가 넉넉한 항목이 한 번 틀렸다고
        마스터가 미뤄지지 않는다"** 이지, "틀려도 무조건 마스터"가 아니다.
      · fast-track 자동확정: 14일 안에 F 가 있으면 확정이 안 된다(`has_f_since`).
      · fast-track 복귀: F만 남긴 통화 2회면 PRACTICING(2.0)으로 되돌아간다.
      · 재료 선별: F 가 있는 항목은 "아는 것"에서 빠지고(`success - failed`),
        F비율이 높으면 브리지 복습 비율이 올라간다(`_struggles`).
      · ⛔ 반면 **placement 굴복(F 1건 → 1.0)은 사실상 죽은 경로**다 — grandfathering 을
        없앤 뒤(bf38921) placement 행이 **새로 생기지 않는다**. 기존 회원에게만 남아 있다.
    """
    # ① 점수 — ⚠ F(−1.0)가 여기로 흘러든다. ④를 뺀 지금 **이것이 F 의 주 방어선**이다.
