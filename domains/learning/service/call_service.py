...
#
# ⚠⚠ **표현학습·프리토킹도 1회로 넣었다(2026-09-10) — 사장님 확인 필요.**
#   ⛔ 안 넣으면 `is_daily_limit_reached` 가 «정의되지 않은 콜타입은 막지 않는다» 로
#     떨어져 두 코스가 **무제한**이 된다. Free 사용자가 표현학습을 하루 종일 돌 수 있고,
#     Live 는 통화당 원가가 나가므로 그건 조용한 비용 구멍이다. 기본값이 무제한인 쪽이
#     1회인 쪽보다 훨씬 위험하다.
#   ⚠ 대신 이 값이면 Free 가 하루에 **3통화**를 하게 된다(normal 1 + 표현학습 1 +
#     프리토킹 1). 그게 의도인지는 사장님이 정할 일이다 — 코스를 가르면 "하루 1통화"의
...
def plan_override_for(db: Session, member_id: int, requested: str | None) -> str | None:
    ...
    ⛔ 통화 엔진 선택(영상/음성·모델·백엔드)과 **조각 수**(`call_fragments_for_plan`, 2026-09-13 사장님: "free 일 때는
      연장하면 안 되고 premium 일 때는 연장되도록")에 쓴다 — 일일 한도·결제·구독은 이 값을 모른다. 롤 조회 실패는 무시(R5 보수 방향).
    """
...
def call_fragments_for_plan(db: Session, member_id: int, plan_override: str | None) -> int:
    """조각 수 — **검증된** plan_override 가 있으면 그 플랜(Free 1 / Premium 3), 없으면 본인 플랜(`call_fragments_for_member`).

    ⭐ 플랜 흉내가 엔진만 바꾸고 조각 수는 본인(admin = Premium 3)대로 두면 «Free 로 통화» 가 5분에 «Keep talking» 을 띄운다 —
...
def active_ongoing_call_id(db: Session, member_id: int) -> int | None:
    ...
    행 잠금을 안 쓰는 이유).
    """
    return CallRepository(db).active_ongoing_call_id(member_id)
...
class CallService:
    ...
    def _quiz_items(self, member_id: int, call: Call) -> list[CallResultQuizItem]:
        """표현학습 퀴즈 결과 — **그 통화에 적힌 스냅샷**을 그대로 읽는다.

        ⛔⛔ **진도 행으로 되짚지 마라**(2026-09-10 QA 로 잡힌 결함). `member_item_progress`
          는 회원×항목 1행이라 `drilled_call_id` 를 **나중 통화가 덮어쓴다**:

            통화 A  「물」 드릴 → 오답        (drilled_call_id = A)
            통화 B  「물」 다시 드릴 → 통과   (drilled_call_id = B)
            ⇒ A 의 결과 화면에서 「물」이 **사라진다**

          그리고 선별이 «드릴했는데 못 끝낸 것» 을 다음 통화 맨 앞에 놓으므로 재드릴은
          예외가 아니라 **정상 경로**다 ⇒ 어제 결과를 오늘 열면 **틀린 항목만 증발하고
          통과 항목만 남는다.** 조용히 틀리는 종류의 버그다.

        ⭐ 그래서 «이 통화에서 무엇을 했나» 는 통화 종료 시점에 `call.expression_result` 로
        ...
        try:
            ...
            logger.warning("표현학습 결과 스냅샷 파싱 실패 call_id=%s", call.call_id)
            return []
        out: list[CallResultQuizItem] = []
