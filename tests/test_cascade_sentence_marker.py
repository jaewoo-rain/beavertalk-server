...
# ── ⭐ 감정 태그 **개수** 관측 (2026-08-16) — 만들기 전에 값어치부터 잰다 ──────
#
# 실측 `감정=` 분포(24h, n=68턴): neutral 55 · angry 7 · surprised 3 · sad 1 · happy 1 · 없음 1
# ⇒ 10턴 중 8턴이 통째로 neutral 이다. 그런 턴은 문장별로 쪼개도 전부 같은 값이라 화면이 안 바뀐다.
#
# ⚠ **그런데 그 80% 를 못 믿을 이유가 있다**: `detect_emotion` 은 `search` 라 **첫 태그만** 읽는다.
#   모델이 `<neutral> … <happy> …` 를 냈으면 happy 는 **사라진다**.
#   ⇒ "모델이 하나만 낸다"와 "우리가 뒤를 버린다"를 지금은 못 가른다. 대사 원문은 로그에
#     안 남기므로(개인정보) **태그만** 세서 그 구분을 만든다.
#
# ⛔ 판정 로직은 안 건드렸다 — 판정과 계측을 같이 바꾸면 무엇이 바뀐 건지 못 가린다.


def test_all_tags_are_counted_not_just_the_first():
    """⭐⭐ **이 한 줄이 이 작업의 전부다** — `search` 가 아니라 전부 센다."""
    assert cr.read_emotion_tags("<neutral> 안녕 <happy> 좋아요") == ["neutral", "happy"]
    assert cr.read_emotion_tags("<happy> 하나뿐") == ["happy"]
    assert cr.read_emotion_tags("태그가 없다") == []


def test_the_detection_itself_is_unchanged():
    """⛔ 판정은 **그대로**다 — 계측을 얹었다고 쓰는 값이 바뀌면 실험이 오염된다."""
    text = "<neutral> 안녕 <happy> 좋아요"
    assert cr.detect_emotion(text) == "neutral", "판정이 첫 태그가 아닌 값을 골랐다"


def test_the_log_pairs_tag_count_with_sentence_count():
    """⚠ 문장 수를 **같이** 찍는다 — 태그 3개/문장 3개와 태그 3개/문장 1개는 전혀 다른 사실이다."""
    line = cs.CascadeSession._emotion_tag_log("<neutral> 하나 <happy> 둘", 2)
    assert "감정태그=2개[neutral,happy]" in line and "문장=2개" in line, line


def test_no_tags_is_a_dash_not_zero():
    """⛔ 0 으로 찍지 않는다 — 오늘 이미 정한 규칙이다(없음과 0 은 다르다)."""
    line = cs.CascadeSession._emotion_tag_log("태그 없는 대사", 3)
    assert "감정태그=-" in line and "문장=3개" in line, line


def test_the_log_never_leaks_the_transcript():
    """⛔ 대사 원문은 **안 남긴다**(개인정보) — 태그 이름만 나간다."""
    secret = "<happy> 제 전화번호는 010-1234-5678 입니다."
    line = cs.CascadeSession._emotion_tag_log(secret, 1)
    assert "010" not in line and "전화번호" not in line, line
