...
# --------------------------------------------------------------------------- #
# 5) Premium 구독 — 카탈로그 전체가 열린다(소유 없이도 통화 가능, D1 2단화: 옛 Max)
# --------------------------------------------------------------------------- #
# ⛔ 여기서 지키는 것: Premium 은 **접근**을 열지 소유를 주지 않는다. member_character
#    행이 생기면 해지 후에도 영구 소유가 되어 되돌릴 수 없다.
...
def test_premium_can_call_unowned_selected_character(db):
...
def test_premium_still_honors_alarm_character(db):
    """Premium 이라도 수신통화는 알람 캐릭터가 이긴다(해석 순서 불변)."""
