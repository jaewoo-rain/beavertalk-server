...
def test_send_then_verify_then_consume(db):
    ...
    svc.send_signup_code(EMAIL)
    svc.verify_code(EMAIL, PURPOSE_SIGNUP, "1234")
    assert svc.consume_verified(EMAIL, PURPOSE_SIGNUP) is True
    # 소비 후에는 다시 없음.
    assert svc.consume_verified(EMAIL, PURPOSE_SIGNUP) is False
...
def test_wrong_code_increments_and_rejects(db):
    ...
    with pytest.raises(Exception):
        svc.verify_code(EMAIL, PURPOSE_SIGNUP, "0000")
...
def test_expired_code_rejected(db):
    svc = EmailVerificationService(db)
    svc.send_signup_code(EMAIL)
    row = svc.repo.get(EMAIL, PURPOSE_SIGNUP)
...
def test_unverified_cannot_be_consumed(db):
    svc = EmailVerificationService(db)
    svc.send_signup_code(EMAIL)  # 발송만, 검증 안 함
    assert svc.consume_verified(EMAIL, PURPOSE_SIGNUP) is False
