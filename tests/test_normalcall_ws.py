...
# ⛔⛔ **지시문 분할 주입 시험을 걷어냈다**(2026-09-01) — 기계 자체가 사라졌다.
#   `LIVE_PERSONA_INJECT` · `build_setup_core` · `split_persona_for_injection` 는
#   2.5 시절의 응급처치였고 3.1 에는 그 제약이 없다. 자세한 경위는
#   `core/persona_prompt.py` 의 「지시문 분할 주입 기계를 걷어냈다」 묘비 주석 참조.
#   ⚠ 되살리려면 그 주석의 실측 3건을 먼저 반증해라.
