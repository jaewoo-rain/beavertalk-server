...
| D2 | ~~**레벨·체크판 UI 비노출**~~ → **레벨은 노출**(2026-07-31 변경) | 당초엔 레벨 숫자까지 감췄으나 마이페이지 개편으로 **종합 레벨 카드**(레벨 + 레벨별 고정 "상위 N%")를 노출한다. 상위 %는 실제 회원 분포 계산이 아니라 고정 표다 — 모수가 수십 명이라 계산하면 한 명 승급에 남의 백분위가 흔들린다(`domains/learning/service/level_percentile.py`). **항목별 상태·진행률은 여전히 비노출** — 그 부분은 서버 내부 기계장치 그대로 |
