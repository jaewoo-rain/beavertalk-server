"""cur_* — 주제별 커리큘럼(차시) 체계 테이블 8개 — **추가만**(기존 테이블 ALTER 0)

설계: docs/20260912_0330_cur-스키마-설계.md (v2 + 컬럼 정리) · 모델 domains/learning/models/curriculum.py
사장님 결정 2026-09-12 #1~#13 + «안 읽는 컬럼은 싣지 않는다»(등급·CEFR·빈도·출처·기능 F코드·모범 대화·성공 조건·가드레일은
시드 JSON 에만). 통화·진도는 cur_* + member + character + call 만 읽는 새 학습 체계.
