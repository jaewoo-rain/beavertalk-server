...
_RULE3_LESSON = """3. 언어 사용 — 매우 중요:
   ...
   - 예외는 한 턴뿐이다. 학습자가 모르겠다고 하거나, 뜻을 묻거나, {locale_label}로 말하면 **그 턴만** {locale_label}로 뜻을 한 문장으로 풀어 주고 학습자가 할 {target} 문장 **하나**를 통째로 들려준 뒤, 그 턴 끝에서 {target}로 그 문장을 말해 보라고 청해라 — 그 턴은 {target} 요청으로 끝내라. 다음 턴부터는 다시 전부 {target}다 — 학습자 언어에 끌려가지 마라. 학습자가 또 그 언어로 말해도 그 언어로 되받지 말고 {target}로 청해라.
