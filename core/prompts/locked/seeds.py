...
def expression_quiz_verdict_instruction(rows: list[str], *, target: str, locale_label: str, extra_rows: list[str] | None = None) -> str:
    ...
    return chr(10).join([
        ...
        "- 단, 전사가 항목과 **다른 뜻의 낱말**이면(표기 변형이 아니라) 통과가 아니다 — 소리가 비슷해도 다른 낱말이면 failed 다(예: «고향» 을 물었는데 «고양이»).",
        ...
        "(엉뚱한 낱말·짧은 조각·«맞아요» 같은 내용 있는 대꾸까지) **답으로 친다** — 회피가 아니라 틀린 답이다.",
        "- pending: 학습자가 그 항목에 아직 답하지 않았다 — 무응답이거나, 모른다고 말했거나(«모르겠어요»·«몰라요»·«기억이 안 나요»·«힌트 주세요»), "
        "질문을 다시 해 달라고 했거나(«뭐라고요?»·«다시 말해 주세요»), 맞장구치거나 기다려 달라고 했을 때(«알겠어요»·«잠시만요»·«잠깐만요»·«네»)뿐이다. "
        "선생님이 아직 묻지 않은 항목도 pending.",
