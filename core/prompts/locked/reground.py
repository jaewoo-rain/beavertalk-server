...
RESUME_SILENT_FIRST_ACTION = (
    ...
    "처음 만난 것처럼 인사하지 말고, «왔냐?»·«어, 왔어?» 같은 시작말도 쓰지 말고, 오늘 무엇을 할지도 다시 묻지 마라. "
...
def build_expression_reground_brief(
    ...
    *,
    ...
    ⭐ `quiz_remaining`(2026-09-13, 실통화 1546): **퀴즈 창이 열린 채** 쪽지가 꽂히면 착지문이 퀴즈를 모른다 —
      ...
      들어가 비버가 방금 것을 또 시켰고, 압축 #3·#4 로 기억이 잘린 3.1 이 같은 두 질문을 3바퀴 돌았다(t35~t43,
      2번째 퀴즈 항목 2개는 아예 안 물어 미판정). ⇒ 남은 문항이 있으면 «다음에 다룰 표현» 줄과 착지문을 **퀴즈
      착지문**으로 바꾼다. None/빈 목록이면 **출력 바이트 동일**(옛 호출부·스냅샷).
    ...
    if remaining_s:
        # ⭐ 퀴즈 중 — 전진 지시 대신 «남은 문항을 마저 내라». 새 표현은 퀴즈 뒤(서버 큐가 창을 닫는다, T16).
        out.append(
            "지금은 퀴즈 중이다 — 아직 안 낸 문항: " + " · ".join(f"«{r}»" for r in remaining_s) +
            ". 이것들을 한 문제씩 마저 내라 — 한 턴에 한 문제, 답을 먼저 말하지 마라. "
            "이미 낸 문제를 다시 내지 마라. 새 표현은 퀴즈가 끝난 뒤다. "
