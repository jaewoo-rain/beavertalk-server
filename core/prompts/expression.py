...
def seed_expression_resume(target_language: str = "한국어") -> str:
    return (
        ...
        "[오늘의 표현] 목록에서 **(통과) 표시가 없는 가장 앞 항목**부터 곧바로 이어가라. "
        "(통과) 가 붙은 것은 다시 가르치지 마라. 통화가 끊겼다 이어졌다는 말은 하지 마라. "
...
def _render_item(n: int, item: dict) -> str:
    """표현 한 줄: `{n}. {surface}` + 뜻/예문 꼬리 (+ 통과 표식).
    ...
    ⚠ `passed` 표식은 조각2·3 에서만 붙는다(첫 조각은 전부 미통과).
    """
    ...
    *,
    ...
    Args:
        items: 이번 통화에서 다룰 표현. 각 dict 는
            {obj: 표면형, des: 뜻|None, ex: 예문|None, quiz_passed: bool}.
            ⭐ 순서가 곧 번호다 — 서버 선별이 낸 순서를 그대로 싣는다.
            ⭐ `quiz_passed=True` 는 **조각2·3** 에서만 온다(이 체인에서 이미 통과한 것).
...
def _items_block(items: list[dict], *, target: str, locale_label: str) -> str:
    ...
    ⭐ 조각2·3: 이미 통과한 번호에 `(통과)` 가 붙고, 아래 한 줄이 **어디서 시작할지**를
      직접 지정한다. 서버가 진도를 갖고 있으므로 사이드카도 JSON 도 필요 없다(D17).
    """
    ...
    if any(it.get("quiz_passed") for it in items):
        # ⛔⛔ «1~7 은 통과했다» 처럼 **번호로** 쓰지 마라(2026-09-10 codex QA 로 정정).
        #   조각2는 새 WebSocket = 새 `_CallState` 라 선별이 **다시 돈다**. 선별이 random 이라
        #   목록이 조각1과 같지 않고, 번호도 옮겨간다 ⇒ 번호로 가리키면 거짓말이 된다.
        #   목록을 고정하려면 코호트를 저장해야 하는데 테이블이 늘고 «매번 랜덤»과 충돌한다.
        #   ⇒ **고정하지 않는다.** 연속성은 선별 정렬(드릴했는데 못 끝낸 것을 앞으로)이 만든다.
        # ⭐ 그래서 지시문은 «어느 번호» 가 아니라 **«표시가 없는 것부터»** 로 말한다 —
        #   목록이 매번 달라져도 항상 참인 문장이다.
        lines.append(
            "★ (통과) 가 붙은 항목은 이미 맞힌 것이다. 그건 다시 가르치지 말고, "
            "**(통과) 표시가 없는 가장 앞 항목부터** 이어서 시작해라."
        )
    lines.append(
