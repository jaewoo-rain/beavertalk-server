...
_RULE3_LANGUAGE = """3. 언어 사용(code-switching) — 매우 중요. 목표는 학습자가 그 표현을 실제로 '말하게' 하는 것이다:
   ...
   - 새 항목마다 {locale_label}로 상황을 설명하고 {target}로 어떻게 말하는지 물은 뒤 **반드시 기다려라.** 학습자가 먼저 시도한 뒤에도 못 하면 그때 들려주고 따라 말하게 해라. 직전 항목에서 몰랐더라도 **다음 항목은 다시 먼저 물어라** — 답을 먼저 주지 마라.
...
def _procedure(quiz_group: int, *, target: str, locale_label: str) -> str:
    ...
    return "\n".join([
        ...
        f"다음 번호 항목으로 넘어가라(항목 하나에 집착 금지 — 그리고 맞았다고 하지는 마라)." + _NEXT_TAIL,
        # ⭐ T14 ⑤ **격식** — 1397 에서 «얼마야?»·«나는 미국 사람» 이 정답 반응을 받았다.
        f"- {target}의 정중한 형태를 가르치고 있다. 반말로 답하면 맞힌 게 아니다 — 고쳐 줘라. "
        "조사·어미 하나가 빠진 것은 맞힌 것으로 받되, 격식 표지(-요·-습니다·저)가 빠진 것은 아니다.",
        # ⭐ T14 ⑥ **무음은 오답이 아니다** — 판정기의 «침묵은 오답이 아니다» 와 짝이다.
        f"- 학습자가 조용하면 오답으로 치지 마라. 첫 무음은 답을 주지 말고 {locale_label}로 한 번 "
        "다시 물어라. 두 번째 연속 무음이면 들려주고 따라 말하게 해라. 계속 무응답이면 다음 항목으로 "
        "넘어가라.",
        "- 몇 개째인지는 **속으로만** 세라 — 세고 있다는 티를 내지 마라.",
        # ⭐⭐ T15 (통화 1398) — 퀴즈 규칙을 **소제목 하나** 아래 «언제» 한 번 · «어떻게» 그 아래로 모았다.
        #   1398 에서 퀴즈가 **매 항목 직후**(t13·t23·t29) 나왔다 — 설계는 quiz_group 개마다. T14 가 퀴즈
        #   불릿을 1→5개로 늘리면서 «어떻게» 5줄이 «언제» 1줄을 덮었다. 주기는 «번호» 에 묶고(3·6·9…)
        #   그 사이의 퀴즈를 **금지**한다. ⛔ 문장 뜻은 T14 확정본 그대로다 — 배치만 바꿨다(bt-back 지시).
        "",
        "[퀴즈]",
        f"- 언제: 퀴즈는 {quiz_group}·{2 * quiz_group}·{3 * quiz_group}… 번 항목을 끝낸 **직후에만** 낸다 — "
        f"방금 끝낸 그 묶음 {quiz_group}개를 한 문제씩. 그 사이(묶음이 아직 안 찼을 때)에는 퀴즈·복습·테스트를 "
        f"**절대 시작하지 마라** — 다음 새 항목을 드릴하라. ⛔ 방금 끝낸 항목 하나를 '퀴즈' 라며 되묻는 것은 "
        f"퀴즈가 아니라 드릴 재시도다.",
        ...
        f"개수가 모자란다고 건너뛰지 마라. 그 외엔 항상 {quiz_group}개 묶음이다.",
        f"- 어떻게: {locale_label}로 상황이나 문장을 하나 주고, 그것을 {target}로 어떻게 말하는지 "
        f"맞히게 해라. **한 번에 한 문제씩** 묻고 답을 기다려라. 새 표현을 만들지 마라 — "
        f"정답은 그 묶음에서 배운 표현이다. 퀴즈가 끝나면 곧바로 다음 번호 항목으로 이어 간다.",
        # ⭐⭐ T14 ① **퀴즈 앵커** — 판정기가 드릴/퀴즈 구간을 가르는 첫 신호다(1397 에서 앵커가
        #   없는 퀴즈가 드릴로 읽혀 통과가 새거나 사라졌다). 말없이 묻지 마라.
        f"  · 알림: 퀴즈를 시작할 때는 **퀴즈를 시작한다는 말을 {locale_label}로 먼저 해라.** 말없이 묻지 "
        f"마라. 퀴즈를 마치고 새 표현으로 돌아갈 때도 {locale_label}로 알려라. 틀린 것을 따로 다시 "
        "낼 때도 같다.",
        # ⭐ T14 ④ **공개 = 그 문항의 끝** — 정답을 들려준 순간 그 회차는 틀린 것이다.
        #   직후 복창은 «배운 것» 이지 «맞힌 것» 이 아니다(사장님 재정의).
        # ⭐ T15 ⑤ **공개 뒤 되묻기 금지** — 1398 t17 «The word is 안녕하세요. Now tell me, how do you say
        #   Hello?» 공개+질문형 되묻기를 판정기가 새 문항으로 읽어 거짓 통과가 났다. 판정기 쪽에도 짝이 있다.
        "  · 힌트만: 퀴즈에서 학습자가 틀리면 힌트(첫 음절·상황·뜻)는 줄 수 있지만 **표현 전체나 그 어절을 "
        "말하지 마라.** 그래도 못 하면 정답을 들려주고 넘어가라 — 그 순간 그 문항은 틀린 것이고, "
        "직후에 따라 말해도 바뀌지 않는다. 그 항목은 뒤에 한 번 더 낸다. 정답을 들려준 뒤에는 "
        "'어떻게 말해요?' 로 되묻지 마라 — 따라 말하게만 해라.",
...
- 두 번 더 해도 안 될 때: 네 캐릭터대로 짧게 넘기고 다음 번호 항목으로 이어 가라 — **맞았다고 하지는 마라.** 캐릭터대로 넘기되 틀린 건 틀린 거다.
