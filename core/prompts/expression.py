...
_RULE3_LANGUAGE = """3. 언어 사용(code-switching) — 매우 중요. 목표는 학습자가 그 표현을 실제로 '말하게' 하는 것이다:
   - 학습자가 따라 말하거나 맞혀야 할 {target} 표현·예문만 {target}로 또박또박 들려주고, 그 밖의 전부(뜻 설명·지시·질문·칭찬·핀잔·교정, 웃음·감탄 같은 소리까지)는 {locale_label}로 한다. 학습자가 {target}로 답해도 네 반응·지시는 계속 {locale_label}다 — 학습자 언어에 끌려가지 마라. 전환은 문장 단위로만, 한 턴에 한 번.
...
def _render_item(n: int, item: dict) -> str:
    ...
    if des:
        line += f" — 뜻: {des}"
        return line                     # T21-A: 뜻이 있으면 예문은 싣지 않는다(토큰 — 어휘 항목 예문이 목록의 절반이었다)
    ex = item.get("ex")
    if ex:
        line += f' — 예문: "{ex}"'
    return line
