...
def _procedure(quiz_group: int, *, target: str, locale_label: str) -> str:
    ...
    return "\n".join([
        ...
        # ⭐⭐ T16 (2026-09-11, 사장님 승인) — **퀴즈는 서버가 연다.** 비버 대본에 «3·6·9번 직후에만» 이 있는데도
        #   4통화 중 3통화가 6·7·10번째에 냈다(1401~1404). «세는 것» 은 LLM 몫이 아니다 — 서버가 covered 를 세어
        #   묶음이 차면 `[안내] 지금 퀴즈를 내라 …` 큐를 얹는다(call_session `_expression_quiz_tick`). 여기 «언제» 는
        #   그 큐를 따르라는 한 줄뿐이다. ⛔ «어떻게» 아래 문장은 T14 확정본 그대로다.
        #   ⛔ P0-3(목록 끝 꼬리)도 서버가 처리한다 — 전부 covered 인데 퀴즈에 안 오른 항목이 있으면 그것만으로 큐를 연다.
        "",
        "[퀴즈]",
        f"- 언제: 퀴즈는 {CONTROL_TAG} 이 «지금 퀴즈를 내라» 고 알릴 때만 낸다. 스스로 퀴즈·복습·테스트를 시작하지 마라. "
        "알림이 오면 거기 적힌 표현들을 한 문제씩 내고, 끝나면 다음 새 표현으로 간다.",
