...
_RULE3_LANGUAGE = """3. 언어 사용(code-switching) — 매우 중요. 목표는 학습자가 그 표현을 실제로 '말하게' 하는 것이다:
   ...
   - 새 항목마다 {locale_label}로 상황을 설명하고 {target}로 어떻게 말하는지 물은 뒤 **반드시 기다려라** — 네가 던진 질문에 스스로 답하지 마라. 학습자가 시도한 뒤에도 못 하면 그때 들려주고 따라 말하게 해라. 직전 항목에서 몰랐더라도 **다음 항목은 다시 먼저 물어라** — 답을 먼저 주지 마라.
   - ★ [착지] 네 턴은 지금 다루는 표현을 학습자가 소리 내어 말하게 하는 요청으로 착지시켜라. 착지의 기준은 화제가 아니라 학습자의 입에서 그 표현이 나오는 것이다 — 소재를 화제 삼아 취향·경험을 묻는 질문은 그 표현 없이도 답할 수 있으니 착지가 아니다."""
...
def _render_item(n: int, item: dict) -> str:
    ...
    if des:
        line += f" — 뜻: {des}"
    # ⚠ T21-A 에서 «뜻이 있으면 예문 생략» 을 넣었다가 fable 검수로 되돌렸다 — D9(어휘·문법 모두 예문)와 어긋나고,
...
def _procedure(quiz_group: int, *, target: str, locale_label: str, has_grammar: bool = False) -> str:
    ...
    ## 지뢰밭(뜻 유지): 「맞았다고 하지 마라」(1398) · 격식(반말 ≠ 정답, 1397) · 새 항목 먼저 묻기(규칙 3) · 마무리·정리 어휘 금지(870).
    """
    del quiz_group
    return "\n".join([
        "[진행 절차]",
        ...
        *(["- [문형] 항목은 문형 이름을 말하게 하지 말고 **연습 문장**을 상황에 맞게 말하게 해라 — 정답은 그 연습 문장이다. "
        ...
        f"- 학습자가 조용하면 오답으로 치지 마라. 첫 무음은 답을 주지 말고 {locale_label}로 다시 묻고, 두 번째 연속 무음이면 "
        "들려주고 따라 말하게 해라 — 계속 무응답이면 다음 항목으로 넘어가라.",
        "",
        "[퀴즈]",
        f"- 퀴즈는 {CONTROL_TAG} 이 «지금 퀴즈를 내라» 고 알릴 때만 낸다 — 스스로 퀴즈·복습·테스트를 시작하지 마라. "
        # ⭐ T21-A 하네스 1420: «알림을 말로 해라» 를 뺐더니 큐 5개 중 2개를 비버가 퀴즈 선언 없이 그냥 되물었다 — 학습자가
        #   퀴즈인지 알아야 한다(앵커 — 1397 의 그 문제). 한 절만 되살린다(≈20 토큰).
        f"알림이 오면 퀴즈를 시작한다는 말을 {locale_label}로 먼저 하고, 거기 적힌 표현들을 한 문제씩, "
        f"{locale_label}로 상황을 주고 {target}로 말하게 하라. "
        "**한 번에 한 문제씩** 묻고 답을 기다려라. 새 표현을 만들지 마라 — 정답은 그 묶음에서 배운 표현이다.",
        "- 틀리면 힌트(첫 음절·상황·뜻)만 — **표현 전체나 그 어절을 말하지 마라.** 그래도 못 하면 정답을 들려주고 넘어가라. "
...
def _items_block(items: list[dict], *, target: str, locale_label: str) -> str:
    ...
    #   ⇒ 처방을 **읽고 있는 그 자리**에 놓는다. (T21-A: 두 문장 이내로 압축 — 뜻 그대로)
    lines.append(
        f"⚠ 적힌 언어는 네가 말할 언어와 무관하다 — 여기 적힌 표현·예문만 {target}로 또박또박 들려주고, "
        f"그것을 꺼내는 말·지시·반응·뜻 설명은 전부 {locale_label}로 해라. "
        "목록의 존재·남은 개수·진행률은 학습자에게 발설하지 마라."
    )
    ...
    lines.append(
        "- 재료를 다 쓴 뒤에도 대화는 그대로 이어진다 — 오늘 다룬 표현들이 서로 어떻게 다르고 언제 쓰는지 "
        f"{locale_label}로 한두 문장만 짚어 주고, 학습자가 그중 하나를 넣은 문장을 직접 만들어 말하게 해라 — "
        "표현을 바꿔 가며 계속 이어가라."
    )
    return "\n".join(lines)
