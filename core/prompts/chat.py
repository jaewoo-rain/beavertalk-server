...
#
# ⛔⛔ R4-a(2026-09-24, bt-back) — 이 시드는 `send_text_turn`(call_session.py) →
#   `send_client_content(role="user", ...)` 로 **user 턴**에 실린다. 옛 문구
#   ("오랜만이에요! 지난번에 …")는 **비버가 할 말을 대괄호 없이 그대로** 담고 있어서,
#   비버가 그걸 학습자가 한 말로 받아 자기 말에 스스로 답했다(선톡 실종 — call 실측).
#   이 코드베이스의 다른 모든 시드(`seed_opening_old`·locked/seeds.py 전부)는
#   **대괄호 지시문**("[통화 시작] …이 [통화 시작] 안내문 자체는 소리 내어 읽지 말고
#   내용만 반영해라")이고, 대괄호로 시작하는 안내문은 `locked/rules.py` 의 일반
#   안전망("대괄호로 시작하는 안내문이 대화 중간에 섞여 들어올 때가 있다 … 소리 내어
#   읽거나 언급하지 말고, 내용만 행동으로 반영하라")이 자동으로 걸러준다. 같은
#   형식으로 맞춘다 — "비버가 할 말"이 아니라 "비버에게 주는 지시"로.
_MEMORY_OPENING_TEMPLATE = (
    "[통화 시작] 오랜만에 학습자에게 다시 전화를 건 상황이다. 지난번에 {topic} "
    "이야기했던 것을 자연스럽게 언급하며 안부를 묻고, 그 일이 어떻게 됐는지 "
    "짧게 물어봐라. 질문만 하고 학습자의 음성 대답을 기다려라. 이 [통화 시작] "
    "안내문 자체는 소리 내어 읽지 말고 내용만 반영해라."
)
