...
def build_streaming_config(
    ...
    if rate is not None:
        ...
        #     ⛔⛔ P2-4(2026-09-24, bt-back QA) — 예전엔 그 호출부가 캐스케이드의
        #     `_speaking_rate()` 였는데, 캐스케이드 엔진 삭제로 그 함수도 없다(이
        #     `build_streaming_config` 자체가 캐스케이드 전용 고아 함수다 — P2-6 정리
        #     대상). 여기서 공통 기본값을 올리면 Chirp 까지 빨라진다는 경고만 유효하다.
