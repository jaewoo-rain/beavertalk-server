...
class Settings(BaseSettings):
    ...
    #   ⚠ "또박또박"은 남긴다 — 그건 속도가 아니라 **또렷함**이고, 어학 앱에서는 필요하다
    #     (초보 학습자는 뭉개진 발음을 못 알아듣는다). 속도와 또렷함은 다른 축이다.
    CASCADE_TTS_STYLE_PROMPT: str = "밝고 다정한 선생님 목소리로, 또박또박."
    ...
    # ⭐ **이 값은 barge-in 판정 기준이 아니다 — STT 장애용 안전망이다**(2026-08-08 재정의).
    #   1.2초였을 때 08-08 통화의 취소 3건이 **전부** 이 경로였다. 전사는 한 글자도 없었는데
    #   "1.2초 이어졌으니 잡음은 아니겠지"로 비버를 죽였다(들린글자 4·13·29자에서 잘렸다).
    #   ⛔ 실측이 이 우회로의 전제를 깼다 — **진짜 말이면 전사가 항상 먼저 이긴다**:
    #     전사로 확정된 4건의 보류→확정  476 · 529 · 539 · 620ms
    #     파이프라인 지연(실측)          723 ~ 914ms
    #   즉 1.2초까지 글자가 없었다 = 글자가 안 나올 소리였다 = 잡음이다.
    #   그래서 **없애지는 않고 밀어 둔다**: STT 가 진짜로 먹통일 때(스트림 롤오버·네트워크)
    #   비버가 사용자 말을 깔고 계속 말하는 **병적 케이스**만 잡는다.
    #   3500ms 근거: 최악의 정상 전사 지연(914ms)의 ~4배 = 정상 경로는 절대 여기 못 온다.
    #   상한은 사용자가 "안 끊긴다"고 느끼는 시간(체감 3~4초)이라 그 위로는 올리지 않는다.
    CASCADE_BARGEIN_SUSTAIN_MS: int = 3500
