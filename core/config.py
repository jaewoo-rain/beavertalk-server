...
class Settings(BaseSettings):
    ...
    #
    # ⭐⭐ **2026-08-14: 300 → 500.** 벤더 침묵창을 500(기본값) → 300 으로 내렸기 때문이다.
    #   ⛔ 이 값은 **벤더가 침묵창을 이미 뺀 뒤의 간격**이다. 사람이 문장 중간에 쉴 수 있는
    #     실제 시간은 `이 값 + 침묵창` 이다:
    #         전  300 + 500 = 800ms
    #         후  500 + 300 = 800ms   ← 정확히 상쇄. 관용도 보존
    #     ⇒ 침묵창만 내리고 이 값을 그대로 두면 관용도가 **600ms 로 후퇴**한다(초보 학습자가
    #       문장 중간에 쉬면 말이 잘린다 — 우리가 제일 피해야 하는 실패다).
    #   ⭐ 그리고 실측이 이걸 요구한다: 벤더가 한 문장을 둘로 쪼갠 그 사건의 간격은 침묵창
    #     500 기준 **84ms** 였다. 침묵창을 300 으로 내리면 같은 사건이 **284ms** 가 된다 —
    #     300 으로 두면 임계의 **95%** 라 사실상 붙어 있다. 500 이면 여유가 216ms 다.
    #   ⚠ 예전 주석의 "임계의 절반 아래" 규칙은 **더 이상 안 맞는다.** 그 규칙이 지키려던 성질은
    #     "진짜 두 번째 발화를 삼키지 않는다"이고, 그 성질의 올바른 단위는 이 값 하나가 아니라
    #     **`이 값 + 침묵창`(= 실제 허용 쉼)** 이다. 회귀를 그 성질로 다시 썼다.
    CASCADE_SPEECH_MERGE_GAP_MS: int = 500
    ...
    # ⛔⛔ **임시(2026-08-19 사장님 지시): 통화 종료 타이밍을 누가 잡나.**
    #     "client" — 프론트가 잡는다. 5분에 "이어서 하시겠습니까?"를 띄우고 **소켓을 닫는다**.
    #                서버 길이 시계는 **안 돈다**(조각 경계에서 비버가 작별하면 안 되므로
    #                이게 설계와 맞다 — 이어하기 §8 "무음 컷 · 주입 0").
    #     "server" — 예전 동작. `call_duration_s` 경과 → 종료 시드 → 작별.
    #
    #   ⚠ 왜 코드를 주석처리하지 않고 스위치로 두나: 그 경로에는 **실제 사고를 기록한
    #     회귀 3건**이 걸려 있다(RC1 소강 스타베이션 · call 197 종료 레이스 · 무음 우선순위).
    #     주석으로 지우면 그 시험들이 같이 죽고, 되살릴 때 무엇을 지켜야 했는지가 사라진다.
    #     스위치면 시험은 "server" 로 계속 돌고 운영만 "client" 로 간다.
    #   ⛔ 이 값이 무엇이든 **절대 백스톱(540초)은 그대로 돈다** — 프론트가 영영 안 닫아도
    ...
    #   연달아 오면 그 뒤로는 응답을 **SILENT** 로 돌린다(= 재개를 촉구하지 않는다).
    #   ⛔ 왜 필요한가: `WHEN_IDLE` 은 "하던 일 끝나면 재개"인데 턴 사이에는 **할 일이 없어
    ...
    # ── P1: 비버가 말한다(LLM → TTS) ──
    # ⭐⭐ **대답 길이는 문장 개수로 센다**(2026-08-12 사장님 결정 "응답은 1~4문장이야").
    #   왜 단위를 바꿨나: 우리는 **이미 문장 단위로** 잘라 TTS 로 흘린다(`SentenceBuffer`).
    #   그런데 길이는 토큰으로 잘랐다 — 단위가 둘이라 경계가 안 맞고 **문장 중간에서 끊겼다**.
    #     실통화 call 938: b1 꼬리 59자 버림 · b2 85자 · b3 **99자**(말한 것의 4배)
    #     짝지은 측정 n=24쌍: 잘림 **11/24(46%)** · 버린 글자 249~259자
    #   ⇒ 상한이 대답 길이 분포의 **한가운데**(평균 82~90자 vs 상한 40토큰 ≈ 90~126자)에
    #     있어서 절반이 걸렸다. 프롬프트 문구로는 못 넘는다(3안 교차 비교에서 효과 미재현).
    #   ⭐ 문장으로 세면 **버림이 구조적으로 0** 이다 — 문장이 끝난 자리에서만 멈춘다.
    #   ⚠ 이 값은 프롬프트 규칙 5("1~4문장")와 **같은 숫자여야 한다.** 두 값이 갈리는 순간
    #     이번 결함(프롬프트 4문장 vs 강제 1문장)이 그대로 재발한다. 회귀로 묶어 뒀다.
    #   0 이면 문장 상한 없음(예전 동작 — 토큰 상한만 남는다).
    CASCADE_LLM_MAX_SENTENCES: int = 4
    # ⭐ **힌트 사이드카를 끌 수 있게**(2026-08-13 사장님 방향: "기능은 하나씩 추가하자").
    #   캐스케이드를 **순정으로 벗겨서** 통화 자체를 먼저 판정한 뒤 기능을 하나씩 올린다 —
    #   우리가 프레임 하나씩 붙일 때 쓴 규율을 **기능 층위로** 올린 것이다.
    #   ⛔ 프론트가 화면에서 안 그리는 걸로는 부족하다: **서버가 안 만들면** 턴당 LLM 호출이
    #     1건 줄고 **로그가 조용해진다.** 순정 판정에 잡음이 없어야 한다.
    #   ⚠ 꺼도 통화는 그대로 돈다 — 힌트는 원래 곁다리다(회귀로 고정).
    #   ⚠ Live 의 힌트(D16)는 이 값과 **무관**하다. 여기는 캐스케이드 전용이다.
    CASCADE_HINT_ENABLED: bool = True
    # ⚠ 토큰 상한은 이제 **길이 조절 수단이 아니라 안전망**이다(2026-08-12). 종결부호를 영영
    #   안 찍는 병리(목록·이모지 폭주·외국어 나열)에서 무한 생성을 막는 마지막 방어선이다.
    #   ⭐ 그래야 `상한잘림` 로그가 **진짜 병리일 때만** 뜬다 — 40 시절엔 절반이 그 표시라
    #     신호가 죽어 있었다. 실측(상한 200·8회): 상한잘림 **0/8**, 꼬리버림 **0자**.
    #   ⚠ 값 40 시절의 근거는 아래에 남겨 둔다(왜 그때 그 값이었는지가 사라지면 안 된다):
    #       상한 없음 평균 15.4초(최악 19.2) · 36 → 8.2초 · 40 → 9.1초 · 64 → 12.6초
    #     ⇒ 그 문제(19초 독백)는 이제 **문장 개수**가 잡는다.
    #   ⚠ 토큰↔글자 비는 **0.34~0.45 토큰per자**로 실측했다. 글자 수로 환산해 쓰지 마라.
    CASCADE_LLM_MAX_OUTPUT_TOKENS: int = 200
    # ⭐ **캐스케이드 대답 LLM 의 리전**(2026-08-13 사장님 지시: "LLM 을 서울로").
    #   비우면 `GCP_LOCATION`(전역)을 따라간다 — 즉 **기본은 지금과 완전히 같다**.
    #   ⛔ `GCP_LOCATION` 을 바꾸면 **Live 도 같이 옮겨간다**(클라이언트가 하나다). Live
    #     네이티브 오디오의 서울 지원 여부를 **확인하지 못했다**(bidi WS 라 단순 호출로 못 잰다)
    #     — 전역 이동은 통화를 죽일 위험이 있어 **캐스케이드만 가른다**.
    #   ⚠ 실측 근거: `generateContent`(gemini-2.5-flash)는 asia-northeast3 에서 **200** 이다.
    #     기대 이득은 `LLM첫조각`(중앙 ~1,100ms)에서 태평양 왕복(~180ms)이 빠지는 것.
    #   ⚠ 힌트·통화후 분석은 **여기 안 딸려온다**(기본 클라이언트를 쓴다) — 근거는 세션 주석.
    CASCADE_LLM_LOCATION: str = ""
    # ⭐⭐ **대답 LLM 커넥션을 살려 둔다**(2026-08-15). 턴마다 TLS 를 새로 맺는 데 ~185ms 를
    #   쓰고 있었다(페어드 A/B 8쌍 전부 개선). `LLM첫조각` 중앙 844ms 의 20% 대다.
    #   ⛔ **30초를 넘기지 마라.** 유휴 커넥션을 GFE 가 먼저 끊으면 그 턴은 **빈 스트림**이 되고
    #     비버가 말을 안 한다(사용자에겐 먹통). 그 사고를 로그로 잡으려고 `gemini_chat` 에
    #     "빈 스트림" WARNING 을 같이 넣었다 — 이 값을 올릴 거면 그 줄부터 확인해라.
    #   ⚠ 0 이면 끈다(예전 동작 = 커넥션 재사용 안 함).
    #   ⚠ **캐스케이드 대답 전용**이다 — Live 는 이 클라이언트를 안 쓴다.
    CASCADE_LLM_KEEPALIVE_S: float = 30.0
