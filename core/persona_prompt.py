...
def seed_opening_lean(target_language: str = "한국어") -> str:
    """분할 주입(LIVE_PERSONA_INJECT)용 **짧은 선톡 시드**.

    ⛔ `seed_opening` 은 스냅샷이 잠겨 있다(tests/test_persona_prompt.py). 그래서 고치지
      않고 **새 심볼**로 둔다 — 기존 경로는 바이트 동일이다.

    ⛔⛔ **왜 짧아야 하나 두 가지**(2026-08-23 실통화 call 1144):
      ① setup 예산: 선톡 시드도 "setup~첫 응답" 총량에 든다. 실측 라운드로빈에서
         코어 380 + 시드 234 = 614자가 9/9 였다. 시드를 줄이면 그만큼 여유가 생긴다.
      ② **인사 중복**: 인사가 13.7초/259자로 길어지자 그 중간(8.60초)에서 `set_face` 가
         불렸고, 직후 인사가 **처음부터 다시** 나갔다. 같은 사고가 call 1117 에도 있다
         (5.6초 호출 → 글자까지 똑같이 두 번). 인사가 짧으면 그 창 자체가 줄어든다.
      ⚠ ②는 **완치가 아니라 창 축소**다. 근본 원인(우리 tool 응답이 재생성을 유발)은
        따로 재야 한다 — 프롬프트의 "첫 인사에서는 부르지 마라"는 이미 안 먹혔다.

    ⚠ 모드 질문의 "두 선택지 모두 대상어 연습"이라는 설명은 **뺐다**. 그건 REST 조각의
      규칙3이 담고 있고, 인사 턴에 그것까지 말하게 하면 다시 길어진다.
    """
    return (
        "[통화 시작] 네가 먼저 전화를 걸었다. **한 문장으로 짧게** 인사하고, 이어서 학습자의 "
        f"모국어로 '오늘 {target_language} 공부할래, 아니면 {target_language}로 수다 떨래?'만 "
        "물어라. 그리고 곧바로 멈춰서 대답을 기다려라. 이 안내문은 읽지 마라."
    )
...
def _study_procedure(is_l1: bool, *, target: str) -> str:
    ...
    return "\n".join([
        ...
        "문장을 직접 만들게 한다(교정은 불변 규칙 4대로)." + _STUDY_NEXT_TAIL,
        "- 단어: ① 뜻을 알려 주고 ② 단어를 또박또박 따라 말하게 — **여기서 멈춰라.** "
        "③ (다음 차례) 학습자가 이미 아는 문법만 쓴 짧은 예문을 만들어 따라 말하게 "
        "해라(새 요소는 그 단어 하나여야 한다)." + _STUDY_NEXT_TAIL,
        "- 통문장: ① 언제 쓰는 말인지 짧게 설명 ② 통문장을 천천히 "
        "또박또박 들려주고 2번 따라 말하게 — **여기서 멈춰라.** "
        "③ 문법·조사를 분해해 설명하지 마라 — 통째로 익히게 한다." + _STUDY_NEXT_TAIL,
        _study_state_procedure(target),
    ])
