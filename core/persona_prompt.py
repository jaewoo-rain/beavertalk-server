...
#
# ⭐ 태그 리터럴을 **한 번도** 쓰지 않는다. 성질만 서술하고 실물은 서버가 쥔다.
#   변천: 3회 노출(따옴표째 진열) → 1회 노출(난수 접미) → 0회.
#   1회로 줄인 뒤에도 새는 것이 실측으로 확인됐다 — 30일간 8건, 그중 3건은 재개 상한
#   (_RESUME_MAX=2)까지 소진했다. 대표 사례 call_id=852(2026-08-01 19:39:35):
#   서버가 종료 시드를 보내기 2분 39초 전에 비버가 "[통화종료:d963] 오늘 공부 너무
#   수고했어! 다음에 또 전화할게!" 를 출력했다. d963 은 그 통화의 난수라 **지시문에만**
#   존재하는 값이다 — 우연한 재현이 아니라 지시문을 그대로 복사한 것이다.
#   난수 접미는 '우연'만 막고 '복사'는 못 막는다. 복사할 원본을 지우는 게 유일한 방어다.
#
#   모델이 태그 문자열을 미리 알 필요는 없다: 종료 시드 본문이 자연어로 자명하고
#   (_close_seed / close_seed_leveltest), 여기서 "대괄호 안내문은 서버 신호이며 낭독하지
#   말고 내용만 행동으로 반영" 이라는 성질을 이미 규정한다. 태그는 서버가 시드를 만들고
#   누출을 탐지(_CONTROL_TAG_RE)하는 데만 쓰인다.
...
[종료 — 서버 전담] 종료 신호는 서버가 대괄호로 시작하는 안내문으로만 주며, 그 신호를 받기 전까지 너의 일은 대화를 이어가는 것이다 — 학습자가 "갈게/끝낼래/bye" 해도 짧게 받은 뒤 새 화제로 이어라(받는 말투는 네 캐릭터대로). 신호가 오면그때만 마무리로: 학습자 마지막 말을 짧게 받고 → 새 질문·화제 없이 따뜻한 작별(평서문 1~2문장, 질문으로 끝내지 마라). ★ 대괄호로 시작하는 서버 안내문은 전부 너에게만 주는 신호다 — 대괄호 안 문구나 "통화 종료"·"종료" 같은 말을 절대 소리 내어 읽거나 입에 담지 마라. 읽지 말고 내용만 행동으로 반영해라.
