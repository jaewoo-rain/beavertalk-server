...
# 응답 길이 규칙(불변 규칙 5) — 기본판과 **짧은판**의 교체 짝.
# ⛔ 기본판은 Live 가 쓰는 그 문장 그대로다(바이트 불변). 짧은판은 **캐스케이드 옵트인**이다.
# 왜 나뉘는가: 캐스케이드는 LLM 출력에 토큰 상한(안전망)이 있고, 프롬프트가 "1~4문장"을
#   시키면 모델은 지시를 따르고 **우리가 중간에서 자른다**(실측 call 938: 글자=27 인데
#   꼬리 99자를 버렸다 — 말한 것의 4배). Live 는 모델이 직접 소리를 내므로 그 문제가 없다.
# ⛔ 상한을 올려 해결하지 않는다 — 대답이 길어져 첫소리·턴이 늦어진다. 상한은 **안전망**이지
#   길이 조절 수단이 아니다. 모델이 스스로 끝맺게 만드는 것이 맞는 해법이다.
_RULE5_LENGTH_DEFAULT = """5. 응답 길이: 매 응답은 1~4문장으로 짧게. 혼자 길게 떠들지 말고 학습자가 말할 차례를 자주 줘라. 통화 시작 시 네가 먼저 말을 건다(선톡)."""

# ⚠ 문구는 **실측으로 고른 것**이다. 3안 교차 비교(턴마다 세 조건을 번갈아 쏴서 시간대
#   드리프트를 상쇄, 8턴 × 2회 = 조건당 n=16, 상한 40토큰 동일):
#       A 기본(1~4문장)      잘림  7/16 · **버린 글자 115** · 평균 90자
#       B 2~3문장            잘림 10/16 · **버린 글자 110** · 평균 94자   ← 제일 나빴다
#       C 1~2문장+끝맺기     잘림  5/16 · **버린 글자  68** · 평균 87자   ← 채택
#   ⭐ B 가 나쁜 이유가 측정으로 갈렸다: 기본이 "1~4문장"이라 **1문장으로 끝낼 수 있는데**
#     "2~3문장"은 하한을 2로 **올린다**. 상한 40토큰에서는 그게 더 긴 대답이다.
#   ⚠ 평균 길이는 90 → 87자로 거의 안 줄었다("너무 짧아도 안 된다"를 지킨다).
#   ⚠ 첫소리는 **이 규칙으로 판정하지 않는다** — 같은 입력의 첫청크가 641~7407ms 로 흔들려
#     호출 간 분산이 지배한다(이미 측정해 둔 사실). 여기서 좋아졌다/나빠졌다고 말할 수 없다.
_RULE5_LENGTH_SHORT = """5. 응답 길이: ★ **1~2문장.** 아주 짧게 말하고 **반드시 문장을 끝맺어라** — 음성 통화라 긴 말은 듣기 힘들다. 한 번에 하나만 가르치고, 마지막은 짧은 질문으로 끝낸다. 혼자 길게 떠들지 말고 학습자가 말할 차례를 자주 줘라. 통화 시작 시 네가 먼저 말을 건다(선톡)."""
...
def build_system_instruction(
    *,
    ...
    # ⭐ 짧은 응답 규칙(캐스케이드 옵트인). 기본값 False 면 **한 바이트도 안 바뀐다**.
    if short_reply:
        template = template.replace(_RULE5_LENGTH_DEFAULT, _RULE5_LENGTH_SHORT)
