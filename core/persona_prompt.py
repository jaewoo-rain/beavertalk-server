...
def new_close_tag() -> str:
    """통화 1건 전용 종료 태그를 만든다 — 모델이 재현하기 어렵게 난수 접미.

    비버가 자기 지시문을 소리 내어 읽어 종료 태그를 스스로 뱉으면, 그 출력이 자기
    컨텍스트에 남아 다음 턴에 종료 신호로 읽힌다(자기충족 루프 — 실측 call_id=706).
    "[시스템]" 같은 일반명사는 우연히도 나오지만 "[통화종료:9f2a]"는 재현이 훨씬 어렵다.
...
#
# ⭐ 태그 리터럴은 **한 곳에서만** 노출한다(이후는 "그 신호"로 지칭). 옛 문단은 리터럴을
#   따옴표째 3회 반복해 낭독 후보로 진열했고, 실제로 비버가 그걸 읽었다.
    ...
    *,
    ...
    Args:
        ...
        close_tag: 이 통화의 종료 신호 태그. 기본 CLOSE_TAG_DEFAULT(스냅샷 기준값).
            런타임은 run_call 이 new_close_tag() 로 만든 통화별 난수 태그를 넘겨,
            시드와 지시문이 **같은 태그**를 쓰게 한다(둘이 어긋나면 종료가 안 먹는다).
...
[종료 — 서버 전담] 이 통화의 종료 신호는 정확히 {close_tag} 로 시작하는 메시지 하나뿐이며, 그 신호를 받기 전까지 너의 일은 대화를 이어가는 것이다 — 학습자가 "갈게/끝낼래/bye" 해도 짧게 받은 뒤 새 화제로 이어라(받는 말투는 네 캐릭터대로). 신호가 오면그때만 마무리로: 학습자 마지막 말을 짧게 받고 → 새 질문·화제 없이 따뜻한 작별(평서문 1~2문장, 질문으로 끝내지 마라). ★ 대괄호로 시작하는 서버 안내문은 전부 너에게만 주는 신호다 — 대괄호 안 문구나 "통화 종료"·"종료" 같은 말을 절대 소리 내어 읽거나 입에 담지 마라. 읽지 말고 내용만 행동으로 반영해라.
...
def build_leveltest_instruction(
    *,
    ...
    Args:
        ...
        close_tag: 이 통화의 종료 신호 태그(기본 CLOSE_TAG_DEFAULT). 반드시
            close_seed_leveltest() 에 넘긴 것과 같은 값이어야 한다.
...
def close_seed_leveltest(close_tag: str = CLOSE_TAG_DEFAULT) -> str:
    ...
    지시문이 "종료 신호는 {close_tag} 뿐"이라고 선언하므로, 시드가 다른 태그로 시작하면
    비버가 그걸 종료로 인식하지 못한다(작별 없이 백스톱 강제 종료로 빠진다).
    """
    return (
