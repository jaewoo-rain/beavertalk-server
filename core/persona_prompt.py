...
#
# ⛔⛔ **예시가 규칙을 배신하고 있었다**(2026-08-16). 옛 문구는 규칙에서 "{target} 를 감싸라"
#   라고 해 놓고 예시로 `오늘은 __How are you?__ 를 배워볼까?` 를 보여줬다 — 일반 통화는
#   target=한국어 / locale=영어 이므로 **감싸면 안 되는 쪽(영어)을 감싼 예시**다.
#   마커 안쪽은 `split_by_language` 가 target 으로 읽으므로 그대로 따르면 **영어가 한국어
#   음성으로** 나간다. 사장님 실측("영어로만 말할 때 한국어 발음으로 말하는 때가 꽤 있었다")과
#   같은 증상이고, `_cross_check` 의 `stats["fixed"]` 가 세고 있던 것이 이것이다.
#   ⚠ 모델은 규칙과 예시가 다르면 **예시를 따른다** — 예시가 곧 규칙이다.
# ⭐ 그래서 예시를 **{target}/{locale_label} 로 자기라벨링**했다. 리터럴 문장을 쓰면 그 예시는
#   특정 (배우는 언어, 모국어) 짝에서만 맞고 다른 짝에서는 또 거꾸로가 된다 — 멀티랭귀지에서
#   같은 계열의 사고가 이미 있었다(2026-07-27 '[대화 모드 — 한국어로 착지]' 헤더 하드코딩).
# ⚠ `{target} 로` 의 군더더기 공백도 같이 없앴다(조사는 붙여 쓴다).
_LANGUAGE_MARKER_RULE = """
[표기 규칙 — 언어 마커]
- {target}로 말하는 부분**만** __이렇게__ 밑줄 두 개로 감싸라. {locale_label} 부분은 감싸지 않는다.
  예) {locale_label}로 하는 리액션·설명 … __{target}로 들려주는 표현·질문__ … 처럼 {target}만 감싼다.
- 서버가 이 경계로 잘라 **부분마다 그 언어의 목소리로 읽는다.** 안 감싸면 {target}가
  {locale_label} 발음으로 읽혀서 {target} 학습에 방해가 된다.
- ⛔ 거꾸로 감싸지 마라. {locale_label}를 감싸면 그 말이 {target} 발음으로 읽혀서 학습자가
  틀린 소리를 따라 하게 된다. **감싸는 것은 언제나 {target}뿐이다.**
...
#
# ⛔⛔ **예시가 규칙 3 을 어기고 있었다**(2026-08-16). 옛 예시는 `<happy> 우와, 정말 잘했어요!`
#   였는데, 규칙 3 은 "칭찬·교정·지시는 계속 {locale_label}로 하고 {target}는 인용만"이라고
#   시킨다(★[학습자 언어에 끌려가지 마라], 2026-07-29 실측 call 784 로 넣은 가드다).
#   즉 **규칙으로 막아 둔 것을 바로 아래 예시가 시범 보이고 있었다** — 게다가 마커도 안 붙어
#   있어 언어 마커 규칙과도 어긋난다. 리액션이 목표어로 물드는 드리프트에 프롬프트가 씨앗을
#   준 셈이다. ⇒ 예시에서 **대사를 빼고 태그 위치만** 보여준다(언어가 섞인 예시는 언어 마커
#   블록이 자기 변수로 갖는다 — 그쪽이 언어의 소유자다).
# ⭐ **neutral 을 기본값으로 유도하던 줄도 고쳤다.** 옛 문구는 "애매하면 <neutral> 을 써라"
#   였다. 가르치는 턴은 대부분 '애매'해 보이므로 이 한 줄이 상시 발동한다 — 실측 24시간
#   68턴 중 neutral 55건(80%)이다. ⇒ neutral 을 '모를 때 찍는 칸'이 아니라 '담담한 순간의
#   감정'으로 다시 정의했다. "목록 밖을 지어내지 마라"(원래 목적)는 그대로 남긴다.
#   ⚠ 이 80% 의 원인이 **모델이 하나만 내서**인지 **우리가 뒤 태그를 버려서**인지는 아직 안
#     갈렸다(`cascade_reply.read_emotion_tags` 주석). 이 수정은 전자를 겨냥한 것이고,
#     후자라면 숫자는 안 움직인다 — 계측이 답을 주기 전까지 효과를 단정하지 마라.
_EMOTION_TAG_RULE = """
[표기 규칙 — 감정 태그]
- 대사 **맨 앞에** 이번 대답의 감정을 하나만 붙여라: {tags} 중에서 고른다.
  예) 대사 맨 앞에 <happy> 처럼 딱 한 번 — 그 뒤부터가 실제로 말할 대사다.
...
- ★ **그 순간 네 기분을 골라라.** <neutral> 은 모르겠을 때 찍는 칸이 아니라 담담하게
  말하는 순간의 감정이다 — 반가움·놀람·안타까움·언짢음이 조금이라도 실렸으면 그 태그를
  써라. 목록에 없는 말은 지어내지 마라.
