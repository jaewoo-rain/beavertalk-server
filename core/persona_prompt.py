...
# 통화 중 주입 1조각의 목표 크기. **0 이하면 자르지 않는다**(전문 1조각).
#
# ⭐⭐ **기본이 「자르지 않음」이다**(2026-08-28 사장님 지시). 규약은 두 턴이다:
#     첫 턴  = 선톡 인사 + 표정 tool (setup 은 짧은 코어)
#     두째 턴 = 페르소나 **전문을 한 번에**
#   ⛔ 왜 바꿨나 — 쪼개면 **학습자가 N번 말할 때까지 비버가 반쪽**이다. 9,067자는
#     1,200자 기준 9조각이라 5분 통화(12~19턴)의 절반을 불완전한 채로 보낸다.
#     그리고 실통화가 그보다 나쁜 것을 보여줬다 — 통화가 끝나도록 **다 못 넣는다**:
#       call 1241  조각 2/8 ⛔미완(남은 6)
#       call 1242  조각 6/9 ⛔미완(남은 3)
#     캐릭터 성격·레벨 프로파일·학습 진도가 영영 안 들어간 채 통화가 끝난다.
#
# ⛔ **1,200자 상한은 2.5 에서 잰 값이다**(2026-08-23):
#     call 1144  조각 1,173자 → 5회 주입 · 96초 완주 · 생존
#     call 1149  조각 2,386자 → 1회 주입한 그 순간 1011 (다른 변경 없음)
#   ⚠ 3.1 에서는 **실패 모양이 다르다.** 1011 로 안 죽고 `interrupted` 로 그 턴을
#     버린다 — 2026-08-28 실통화에서 1,173자 주입이 **6/6 전부** 턴을 끊었고
#     비버 대사가 "Great choice! Let's" / "Right on!" 처럼 토막났다.
#     ⇒ 크기가 아니라 **주입 행위 자체**를 3.1 이 끼어들기로 본다. 그러면 조각을
#       잘게 나눌수록 끊기는 횟수만 늘어난다. 한 번에 넣는 편이 낫다.
#
# ⚠ 3.1 에서 전문 1회 주입은 **아직 실통화로 안 쟀다.** env 로 되돌릴 수 있게 열어 둔다.
LIVE_PERSONA_CHUNK_CHARS = 0  # 0 = 자르지 않음(전문 1조각)
...
# ⭐⭐ **순서 규칙이 여기 있어야 한다**(2026-08-31 실측으로 추가).
#   ⛔ 예전엔 "표정이 바뀌면 그 말을 하면서 함께 불러라" 한 줄뿐이었는데, 그 문구는
#     `ed35e6f`(08-24)가 **이미 실패 판정한 것**이다 — 「차례 맨 앞 호출」을 못 막는다.
#   실측(call 1252): 9회 호출 중 **7회가 턴누적오디오=0.00초**. 그 결과가 둘이다:
#     ⓐ 마커가 직전 턴 끝에 꽂혀 **엉뚱한 턴에서** 뜬다
#     ⓑ 연달아 부르면 위치가 같아져(at=2270) 뒤엣것이 앞엣것을 **즉시 덮는다**
#        happy→neutral 짝이 오면 아무 표정도 안 보인다 — 8개 중 **3개만** 화면에 떴다
#   ⭐ 08-24 실측에서 이 문구가 먹혔다: 첫 호출이 0.00초 → 7.50 / 9.82 / 10.78초로 밀림.
#   ⚠ 긴 지시문(_FACE_TOOL_RULE)에는 원래 있었는데, LIVE_PERSONA_INJECT 로 그것이
#     setup 밖으로 나가면서 **첫 턴부터 규칙이 안 닿게** 됐다. 코어에도 심는다.
#   ⚠ **380자 예산 안에 넣어야 한다.** 원래 문구를 그대로 옮겼더니 최악 404자로 넘쳤다
#     (494자에서 생존이 9/9 → 6/9 로 무너지는 값이다). 뜻은 남기고 글자만 깎았다.
_SETUP_CORE_FACE = (
    "\n표정: 몇 마디 말한 뒤 바뀌는 지점에서 set_face 를 불러라."
    " 차례 맨 앞·첫 인사에서는 부르지 마라."
)
...
def split_persona_for_injection(
    ...
    if not full:
        return []
    # ⭐ 0 이하 = 자르지 않는다(전문 1조각). 두 턴 규약의 기본값이다 — 위 상수 주석 참조.
    if chunk_chars <= 0:
        return [full]
    out: list[str] = []
    cur = ""
