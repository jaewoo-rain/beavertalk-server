...
#
# ⛔⛔ **1,200자를 넘기지 마라 — 실통화가 증명했다**(2026-08-23).
#   call 1144: 조각 1,173자 → 5회 주입 · 96초 완주 · 생존
#   call 1149: 조각 2,386자 → **1회 주입한 그 순간 1011** (다른 변경 없음)
#   "조각이 8개라 다 차려면 학습자가 8번 말해야 한다"는 이유로 2,400자로 올렸다가
#   통화가 즉사했다. 조각 수가 많은 건 품질 문제지만 조각이 큰 건 **생존 문제**다.
#   ⚠ 오프라인 하네스에서는 4,757자 통째도 100% 살았다 — 실서비스에서만 죽는다.
#     하네스가 못 잡는 구간이 또 있다는 뜻이니, 이 값은 실통화로만 올려라.
